...
static esp_err_t gpio_pulse_clock(const gpio_num_t clock)
{
    esp_err_t ret = ESP_FAIL;

    ESP_GOTO_ON_ERROR(gpio_set_level(clock, 1), fail, TAG, "gpio_set_level %s", esp_err_to_name(ret));
    esp_rom_delay_us(PULSE_CLOCK_DELAY_US);
    ESP_GOTO_ON_ERROR(gpio_set_level(clock, 0), fail, TAG, "gpio_set_level: %s", esp_err_to_name(ret));
    ret = ESP_OK;
fail:
    return ret;
...
static esp_err_t gpio_pulse_reg_clock(const gpio_num_t reg_clock)
{
    esp_err_t ret = ESP_FAIL;

    ESP_GOTO_ON_ERROR(gpio_set_level(reg_clock, 1), fail, TAG, "gpio_set_level %s", esp_err_to_name(ret));
    esp_rom_delay_us(PULSE_REG_CLOCK_DELAY_US);
    ESP_GOTO_ON_ERROR(gpio_set_level(reg_clock, 0), fail, TAG, "gpio_set_level %s", esp_err_to_name(ret));
    ret = ESP_OK;
fail:
    if (ret != ESP_OK) {
...
esp_err_t shiftregister_gpio_transfer(const shiftregister_config_t *config, uint8_t *data, const size_t size)
{
    ...
    for (int index = 0; index < size; index++) {
        ...
        for (int bit_index = 0; bit_index < 8; bit_index++) {
            ...
            ESP_GOTO_ON_ERROR(gpio_pulse_clock(config->clk_io_num), fail, TAG, "gpio_pulse_clock: %s", esp_err_to_name(ret));
            ESP_GOTO_ON_ERROR(gpio_write(config->data_io_num, 0), fail, TAG, "gpio_write: %s", esp_err_to_name(ret));
        }
        ESP_GOTO_ON_ERROR(gpio_pulse_reg_clock(config->rclk_io_num), fail, TAG, "gpio_pulse_reg_clock: %s", esp_err_to_name(ret));
    }
    ret = ESP_OK;
fail:
    return ret;
